...
> **September 2026 compatibility:** Google is rolling out Interactive Learning Overviews and additional quiz formats such as short answer, multiple select, and fill in the blank. Existing Video Overview downloads remain format-agnostic, quiz export now tolerates multiple schema shapes, and every Studio artifact is also stored under `artifacts/_raw/` so newly introduced formats are not silently discarded. A raw snapshot preserves the server payload for later re-parsing; it does not claim lossless rendering of a format whose schema has not yet been observed.
