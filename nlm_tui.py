"""
nlm-tui: NotebookLM TUI ツール (Windows / Linux fallback)

主なキー:
  ↑/↓ (j/k) : 移動
  Space      : 選択/解除
  Enter      : 詳細ツリー表示
  b          : 選択ノートブックを一括バックアップ（未選択なら現在行）
  u          : アップロードメニュー（新規作成/既存追加）
...
def _backup_notebook(
    ...
    if selection.artifacts:
        if retry_plan is not None:
            ...
            # Preserve raw payloads for non-completed rows without counting them
            # as failed downloads. Completed rows are exported below.
...
def _run_backup_batch(
    ...
    for idx, target in enumerate(targets, 1):
        ...
        on_progress(idx, len(targets), nb_title, 1.0, "完了", finished)
...
class TerminalFallbackTUI:
    ...
    def _read_key_posix(self) -> str:
        ...
        try:
            ...
            if self._posix_pending_esc:
                ...
                # ESC 単体だった場合は次の通常キーとして扱う

            # 一部端末では矢印の末尾文字(A/B/C/D)だけが遅れて届くことがある
            # その場合は通常キー(a/b/c/d)として扱わず矢印に解釈する
            if ch in ("A", "B", "C", "D"):
            ...
            if ch == "\x1b":
                ...
                ready, _, _ = select.select([sys.stdin], [], [], 0.18)
                if not ready:
                    # Linux では ESC を終了キーに使わないので保留して誤判定を防ぐ
            ...
            # まれに ESC が先に消費されて '[' から始まることがあるため救済
            if ch in ("[", "O"):
    ...
    def _draw_list(self):
        ...
        lines.append(
            "↑/↓:移動  Space:選択  Enter:詳細  b:バックアップ  u:アップロード  "
            "x:失敗再試行  f:フィルタ  a:全選択  r:再読込  q:終了"
        )
        lines.append("-" * 80)

        if not self.notebooks:
            lines.append("ノートブックがありません。r で再読込、q で終了。")
            lines.append("")
    ...
    def _browse_lines(self, title: str, lines: list[str]):
        ...
        while True:
            ...
            view: list[str] = [title, "↑/↓:スクロール  q/Enter:戻る", "-" * 80]
    ...
    def _run_upload_flow(self, target_notebook_id: str, target_notebook_title: str):
        paths_raw = self._prompt_line(
            ...
            "ファイル/フォルダのパスを ';' 区切りで入力（空ならスキップ）",
        )
        urls_raw = self._prompt_line(
            f"Upload -> {target_notebook_title}",
            "URL を ';' 区切りで入力（空ならスキップ）",
        )
        ...
        if not files and not urls:
            self.status = "アップロード対象がありません"
            return

        if paths and not files and not urls:
            self.status = "指定パスからアップロード対象ファイルを見つけられませんでした"
            return
        ...
        if summary["files_fail"] or summary["urls_fail"]:
            self.status = "アップロード完了（失敗あり）"
        else:
            self.status = "アップロード完了"
    ...
    def _upload_menu(self):
        while True:
            ...
            if key in ("q", "enter") or self._is_esc_action(key):
                return
            if key == "1":
                title = self._prompt_line("新規ノートブック作成", "ノートブックタイトルを入力")
                if not title:
                    self.status = "タイトルが空です"
                    return
                try:
                    notebook_id = self.client.create_notebook(title)
                except NotebookLMError as e:
                    self.status = f"作成失敗: {e}"
                    return
    ...
    def _run_targets(self, targets: list[dict], mode_label: str):
        ...
        if failures:
            ...
            self.status = f"{mode_label}完了: 失敗 {queued} 件（x で再試行）"
        else:
            self.status = f"{mode_label}完了: すべて成功"
    ...
    def _run_backup(self):
        ...
        self._run_targets(targets, "バックアップ")

    def _retry_failures(self):
        if not self.last_failures:
            self.status = "再試行対象がありません"
            return
        targets = []
    ...
    def _reload(self):
        try:
            ...
            self.offset = 0
            self.selected.clear()
            self.status = f"再読込完了: {len(self.notebooks)} 件"
            self._log(f"Reload notebook list count={len(self.notebooks)}")
        except NotebookLMError as e:
            self.status = f"再読込失敗: {e}"
    ...
    def _configure_filter(self):
        while True:
            self._render(
                [
                    "バックアップ対象フィルタ",
                    "1:Sources  2:Artifacts  3:Notes  4:Mindmaps  Enter/q:戻る",
                    "-" * 50,
            ...
            if key == "1":
                if not _toggle_selection_component(self.selection, "sources"):
                    self.status = "最低1つは ON のままにしてください"
            elif key == "2":
...
def main() -> int:
    ...
    parser.add_argument("--log", type=str, default=None, help="ログファイルのパス (default: <output>/nlm_tui.log)")
    args = parser.parse_args()

    if not sys.stdin.isatty() or not sys.stdout.isatty():
        print("[ERROR] nlm-tui は対話式ターミナルで実行してください。", file=sys.stderr)
        return 1

    if sys.platform == "win32" and msvcrt is None:
        print("[ERROR] Windows のキー入力モジュール(msvcrt)を読み込めません。", file=sys.stderr)
        return 1

    if sys.platform != "win32" and (termios is None or tty is None or select is None):
        print("[ERROR] POSIX のキー入力モジュール(termios/tty/select)を読み込めません。", file=sys.stderr)
        return 1
    ...
    try:
        ...
        print(f"[ERROR] ノートブック一覧の取得に失敗: {e}", file=sys.stderr)
        return 1
